...
const main = async () => {
  const feed = new RSS({ title: 'Movies' })
  ...
  const media = await fetchMedia(tvShowsUrl)
  const titles = media.map((edge: any) => edge.node.title.titleText.text)
  titles.forEach((title: string) => {
    feed.item({ title })
  });
  const xml = feed.xml()
  console.log(xml)
  fs.writeFileSync('public/movies.xml', xml)
}
